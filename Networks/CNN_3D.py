...
def last_blck(frst_block, previous_block_wp, previous_block_pz, nfilter, k1, k2, k3, actvfc, p, add):
    ...
    #Segment PZ
    previous_block_pz=UpSampling3D(size=(2,2,1))(previous_block_pz)
    previous_block_pz=Conv3D(filters=nfilter, kernel_size=(2,2,1), padding='same', activation=actvfc,
    ...
    res= Add()([x1, x3])
    outputPZ= Conv3D(filters=1, kernel_size=(1,1,1), activation='sigmoid', kernel_initializer='he_uniform', name="PZ" )(res)

    #Segment WP
    previous_block_wp=UpSampling3D(size=(2,2,1))(previous_block_wp)
    previous_block_wp=Conv3D(filters=nfilter, kernel_size=(2,2,1), padding='same', activation=actvfc,
    ...
    x= SpatialDropout3D(p)(x)
    x4=Conv3D(filters=nfilter, kernel_size=(k1,k1,k1), padding='same', kernel_initializer='he_uniform')(x)
    x4=BatchNormalization()(x4)
    x4= Activation(actvfc)(x4)

    x5=Conv3D(filters=nfilter, kernel_size=(k2,k2,k2), padding='same', kernel_initializer='he_uniform')(x4)
    x5=BatchNormalization()(x5)
    x5= Activation(actvfc)(x5)

    x6=Conv3D(filters=nfilter, kernel_size=(k3,k3,k3), padding='same', kernel_initializer='he_uniform')(x5)
    x6=BatchNormalization()(x6)
    x6= Activation(actvfc)(x6)

    res2= Add()([x4, x6])
    res2=Concatenate()([res2, res])
    x7=Conv3D(filters=nfilter, kernel_size=(k3,k3,k3), padding='same', kernel_initializer='he_uniform')(res2)
    x7=BatchNormalization()(x7)
    x7= Activation(actvfc)(x7)
    output= Conv3D(filters=1, kernel_size=(1,1,1), activation='sigmoid', kernel_initializer='he_uniform', name="main" )(x7)
...
def res_downsampling(previous_block, nfilter, k1, k2, k3, actvfc, p):
    ...
    #print('previous_block.shape ', previous_block.shape)
    x= MaxPooling3D(pool_size=(2,2,1), strides=(2,2,1))(previous_block)
    x= SpatialDropout3D(p)(x)
...
def res_upsampling(downsampling_block, previous_block_wp, previous_block_pz, nfilter, k1, k2, k3, actvfc, p, add):
    ...
    x4= SpatialDropout3D(p)(x4)
    x4=Conv3D(filters=nfilter, kernel_size=(k1,k1,k1), padding='same', kernel_initializer='he_uniform')(x4)
    x4=BatchNormalization()(x4)
    x4= Activation(actvfc)(x4)
...
def get_3DCNN(h=512, w=512,p=0.5,k1=3,k2=3, k3=3, nfilter=32,actvfc='relu', blocks=9, slices=128, channels=1, add=True):
